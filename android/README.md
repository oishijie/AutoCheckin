...
>
> 第 ④ 步之后（定位签到 / 拍照签到 / 提交）**待 20:00 签到窗口开启后实测补齐**。
...
| 层次 | 动作 | 说明 |
|---|---|---|
| 第一层 | `wait` 超过 **8 秒**仍挂着「加载中...」→ 判定卡死 → **BACK 退回列表 → 重新点开卡片** | 不退出 App，实测几秒即完成 |
| 第二层（兜底） | 本页重进仍失败 → 整体重试（重试前 BACK ×4 回收现场，再完整重走入口） | 默认重试 3 次（共 4 轮） |
