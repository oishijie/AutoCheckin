...
# ⑦ 按快门（截图识别白色快门，见下文「图像识别」一节）
shutter|0|0|0|15000

# ⑦+ 等图片编辑页打开（别省：实测快门点完 0.7 秒就去点「完成」等于没点）
sleep|2500|0|0|1000

# ⑧ 编辑页点「完成」，把照片回传给签到页
id|tv_done|0|0|15000

# ⑨ 等照片真的回传完成：\ue62b（相机图标）→ \ue605（已上传缩略图）
...
> 系统手势安全线更低到 2129），**坐标点击必被系统手势区吞掉**，只能靠无障碍 `ACTION_CLICK` ——
...
>
> ⚠️ **已知局限**：按钮在**没有照片**时同样是可点的蓝色，点下去 H5 会静默拒绝（实测点击后页面文案、拍照图标全无变化）。
> 也就是**「点到了」≠「签上了」**。真正的成功判据（页面是否变成「已签到」）需一次真实签到才能确认 ——
> 目前流程跑通后会自动 dump 界面进日志，届时可据此补上校验。
...
```
kind|value|contains|optional|waitMs
```
