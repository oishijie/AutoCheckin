...
> | ⚠️ 已提交、未确认 | 提交发出后某步失败 | **不报成功、不重跑**，dump 现场待人工核对 |
...
>
> 📌 **仍未闭环**：⑱ 目前只能靠「弹窗」判成功，而**「已签到」页面长什么样还不知道**
> （没有真实签到后的页面样本）。等首次实签采集到页面特征，再把判据从「弹窗」扩到「页面状态」。
> 在那之前**故意保持 `optional=0`** —— 拿不到弹窗就不敢报成功，因为**假成功比假失败更危险**
> （用户以为签上了，当天机会却已错过）。
