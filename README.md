...
| 图像识别兜底 | 拍照快门没有稳定文本，改用截图做**连通域白斑识别**（面积 / 填充率 / 长宽比三重判据 + 连拍双帧取交集），识别失败退回**上次成功坐标** |
| 卡住自动救 | 页面偶发卡在「加载中」时，先 BACK 退回重开签到卡试满 3 次，仍不行就重启目标 App 进程；这一组重复 2 遍 |
| 后台跳转豁免 | 挂一个 1×1 隐形窗口拿到 Android 12+ 的后台启动白名单，否则定时触发时跳转会被系统静默丢弃 |
| 每日定时 | AlarmManager 精确闹钟 + 开机自动重排，熄屏也能跑 |
| 结果通知 | 签到成功 / 已签到 / 待确认 / 失败，四种结果各推一条到微信（走自建 Worker，可关） |
...
│  ├─ ShutterFinder.kt               截图识别拍照快门（白斑判据 + 双帧交集 + 历史坐标兜底）
...
│  ├─ Pusher.kt                      签到结果推送到微信（HttpURLConnection，零依赖）
...
| `id` / `desc` / `cls` | 按控件 id / 无障碍描述 / 类名找 | `tv_done` |
| `uni` | 按字体图标码点找（PUA 字符） | `e62b` |
| `uiwait` | 等某个控件出现（有超时） | `e62b` |
| `viswait` | 等文字出现、且真的渲染出尺寸（不触发重试） | `提交签到` |
| `done` | **终止信号**：命中即代表今天已完成签到，整轮立即收工 | `已签到` |
| `xy` | 按坐标点，支持百分比 | `50%,91%` |
| `shutter` | 截图识别白色快门圆盘并点击；识别失败按「上次成功坐标 → 默认比例」回退 | 空 |
| `scroll` | 滚动 | 空 |
| `sleep` | 静置等待 | `1500` |
...
>
> **`done`**：命中该文本即代表**今天已经签过了**，整轮立即以成功收尾 ——
> 不拍照、不提交、不重试。它专治「用户自己手动签过、App 却不知道」这种情况：
> 那时签到页是「已完成」态（有 `已签到` 和照片，但**没有** `重新定位` 按钮），
> 照旧往下走必然白跑 4 轮，还会冒二次提交的风险。
